...
public class EssentialConversion {
    ...
    public static final int[] getAsciicode(String input) {
	...
	public static String binarytostr(String input) {
		...
		for(int i=0;i<input.length()-1;i+=8){
			charValue=Integer.parseInt(input.substring(i, i+8),2);
		       output += EssentialConversion.getAscii(charValue);
		}
		return output;
	}
	...
	public static String tobinary(String input) {
			String temp,output="";
			int[] stringValues = EssentialConversion.getAsciicode(input);
			for(int i=0 ; i<stringValues.length; i++){
				temp= Integer.toBinaryString(stringValues[i]);
				while(temp.length() !=8){
			           temp = "0"+temp;
			       }
				output +=temp;
